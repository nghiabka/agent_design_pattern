...
from agentic_rag.api_client import BackendClient
...
def backend_get(path: str, timeout: int = 10) -> dict[str, Any] | None:
    return backend_client().get(path, timeout=timeout)


def backend_chat(question: str) -> dict[str, Any]:
    return backend_client().chat(
        question=question,
...
def render_response_metrics(message: dict[str, Any]) -> None:
    response_time = message.get("response_time_seconds")
    if response_time is None:
        trace = message.get("trace") or {}
        response_time = trace.get("elapsed")

    if response_time is not None:
        st.caption(f"Response time: `{float(response_time):.2f}s`")
...
def render_message(message: dict[str, Any], max_rounds: int | None = None) -> None:
    with st.chat_message(message["role"]):
        ...
        if message["role"] == "assistant":
            render_response_metrics(message)
    ...
    with st.chat_message("assistant"):
        ...
        with st.spinner("Backend đang chạy Agentic RAG..."):
            try:
                ...
                response_time = payload.get("response_time_seconds")
